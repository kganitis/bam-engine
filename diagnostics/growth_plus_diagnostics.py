"""
==============================================
Growth+ Comprehensive Diagnostics Dashboard
==============================================

Deep diagnostic visualization of every meaningful variable and relationship
in the BAM Growth+ model (Section 3.8). This script captures everything
from the baseline diagnostics plus R&D-specific dynamics:

- All 7 core roles (~30 per-agent fields) with proper capture timing
- R&D extension role (sigma, intensity, productivity increment, fragility)
...
- Productivity growth and wage co-movement dynamics
- Stock-flow consistency checks

Output: 15 multi-panel figures saved to ``diagnostics/output/``.
...
For non-interactive (CI/headless) usage::
    ...
    MPLBACKEND=Agg python diagnostics/growth_plus_diagnostics.py
"""
...
C_RND = "#10B981"  # Emerald - R&D / Productivity Growth
...
def save_figure(fig, fig_num, name):
    ...
    filename = f"growth_plus_{fig_num:02d}_{name}.png"
...
COLLECT_CONFIG = {
    ...
    "RnD": ["sigma", "rnd_intensity", "productivity_increment", "fragility"],
    "CreditMetrics": [
        "n_loans",
        "total_credit",
        "market_share",
    ...
    "capture_timing": {
        ...
        "Borrower.retained_profit": "firms_deduct_rnd_expenditure",
        # Lender
        ...
        # RnD
        "RnD.sigma": "firms_compute_rnd_intensity",
        "RnD.rnd_intensity": "firms_compute_rnd_intensity",
        "RnD.productivity_increment": "firms_apply_productivity_growth",
        "RnD.fragility": "firms_compute_rnd_intensity",
        # CreditMetrics
...
# RnD (n_periods x n_firms)
rnd_sigma = results.get_array("RnD", "sigma")
rnd_intensity = results.get_array("RnD", "rnd_intensity")
rnd_increment = results.get_array("RnD", "productivity_increment")
rnd_fragility = results.get_array("RnD", "fragility")
...
# Minimum wage history (from economy data if available, else estimate)
min_wage_data = results.economy_data.get("min_wage", None)
if min_wage_data is not None:
    min_wage = np.array(min_wage_data)
elif len(_min_wage_history) > 0:
    min_wage = np.array(_min_wage_history)
else:
    # Fallback: not available
    min_wage = np.full(N_PERIODS, np.nan)
...
weighted_prod = np.sum(labor_productivity * production, axis=1)
avg_productivity = np.where(total_gdp > 0, weighted_prod / total_gdp, 0.0)
...
safe_prod_prev = np.where(avg_productivity[:-1] > 0, avg_productivity[:-1], 1.0)
productivity_growth = np.where(
    avg_productivity[:-1] > 0,
    np.diff(avg_productivity) / safe_prod_prev,
    0.0,
)
...
# R&D aggregates
avg_sigma = np.mean(rnd_sigma, axis=1)
avg_rnd_intensity = np.mean(rnd_intensity, axis=1)
avg_rnd_increment = np.mean(rnd_increment, axis=1)
avg_rnd_fragility = np.mean(rnd_fragility, axis=1)
pct_rnd_active = np.mean(rnd_sigma > 0, axis=1) * 100  # % of firms doing R&D
total_rnd_expenditure = np.sum(rnd_sigma * net_profit, axis=1)

# Wage-to-productivity ratio
wage_prod_ratio = np.where(avg_productivity > 0, real_wage / avg_productivity, 0.0)
...
# Diagnostic: verify CreditMetrics events ran
print(f"\nCreditMetrics check: _min_wage_history has {len(_min_wage_history)} entries")
print(f"  cm_n_loans sum period 10: {np.sum(cm_n_loans[min(10, N_PERIODS - 1)]):.0f}")
print(f"  cm_hhi period 10: {cm_hhi[min(10, N_PERIODS - 1), 0]:.4f}")

# Early dynamics (periods 0-50) to understand degenerate transition
print("\n--- Early Dynamics (periods 0-50) ---")
for t in [0, 5, 10, 20, 30, 50, 100, 200]:
    if t < N_PERIODS:
        print(
            f"  t={t:4d}: GDP={total_gdp[t]:8.1f}  Unemp={unemployment[t] * 100:5.1f}%  "
            f"AvgProd={avg_productivity[t]:.4f}  AvgPrice={avg_price[t]:.3f}  "
            f"R&D_active={pct_rnd_active[t]:.1f}%  "
            f"AvgSigma={avg_sigma[t]:.4f}  "
            f"NetWorth={np.mean(net_worth[t]):.2f}  "
            f"GrossProfit={np.mean(gross_profit[t]):.2f}  "
            f"NetProfit={np.mean(net_profit[t]):.2f}"
        )
...
print(
    f"  Productivity growth: {(avg_productivity[-1] - avg_productivity[bi]) / max(avg_productivity[bi], 1e-10) * 100:.1f}%"
)
print(f"  Mean R&D sigma: {np.mean(avg_sigma[bi:]):.4f}")
print(f"  Mean R&D intensity: {np.mean(avg_rnd_intensity[bi:]):.6f}")
print(f"  Pct firms doing R&D: {np.mean(pct_rnd_active[bi:]):.1f}%")
print("\nGenerating 15 diagnostic figures...")
...
fig.suptitle("Figure 1: Production & Output (Growth+)", fontsize=14, y=0.995)
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 2: Labor Market (Growth+)", fontsize=14, y=0.995)
...
# (2,0) Minimum wage (or real wage if not enough data)
ax = axes[2, 0]
if len(min_wage) >= N_PERIODS and not np.all(np.isnan(min_wage)):
    ax.plot(t_post, min_wage[bi:], color=C_LABOR, linewidth=1)
    style_ax(ax, "Minimum Wage", ylabel="Wage")
else:
    ax.plot(t_post, real_wage[bi:], color=C_LABOR, linewidth=1)
    style_ax(ax, "Real Wage (min_wage unavailable)", ylabel="Wage")
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 3: Credit Market (Growth+)", fontsize=14, y=0.995)
...
fig.suptitle("Figure 4: Prices & Inflation (Growth+)", fontsize=14, y=0.998)
...
fig, axes = plt.subplots(3, 2, figsize=(14, 15))
fig.suptitle("Figure 5: Goods Market & Consumption (Growth+)", fontsize=14, y=0.998)
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 6: Revenue & Profits (Growth+)", fontsize=14, y=0.995)
...
plot_band(ax, t_post, m[bi:], s[bi:], lo[bi:], hi[bi:], C_REVENUE)
ax.axhline(0, color="gray", linewidth=0.5, linestyle=":")
style_ax(ax, "Retained Profit per Firm (after R&D)", ylabel="Amount")
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 7: Credit Network (Growth+)", fontsize=14, y=0.995)
...
fig.suptitle("Figure 8: Bankruptcy & Entry (Growth+)", fontsize=14, y=1.0)
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 9: Macroeconomic Curves (Growth+)", fontsize=14, y=0.995)
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 10: Derived Macro Ratios (Growth+)", fontsize=14, y=0.995)
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle(
    "Figure 11: Distribution Dynamics \u2014 Firms (Growth+)", fontsize=14, y=0.995
)
...
fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle(
    "Figure 12: Distribution Dynamics \u2014 Households & Banks (Growth+)",
    fontsize=14,
    y=0.995,
)
...
fig, axes = plt.subplots(2, 2, figsize=(14, 10))
fig.suptitle("Figure 13: Stock-Flow Consistency Checks (Growth+)", fontsize=14, y=1.0)
...
# %%
# Figure 14: R&D Dynamics (Growth+ Specific)
# -------------------------------------------

fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle("Figure 14: R&D Dynamics (Growth+)", fontsize=14, y=0.995)

# (0,0) Average R&D sigma (share of profits invested in R&D)
ax = axes[0, 0]
m, s, lo, hi = weighted_stats(rnd_sigma)
plot_band(ax, t_post, m[bi:], s[bi:], lo[bi:], hi[bi:], C_RND)
style_ax(ax, "R&D Share (sigma) per Firm", ylabel="Share")

# (0,1) R&D intensity (expected productivity gain mu)
ax = axes[0, 1]
m, s, lo, hi = weighted_stats(rnd_intensity)
plot_band(ax, t_post, m[bi:], s[bi:], lo[bi:], hi[bi:], C_RND)
style_ax(ax, "R&D Intensity (mu) per Firm", ylabel="Intensity")

# (1,0) Productivity increment per firm
ax = axes[1, 0]
m, s, lo, hi = weighted_stats(rnd_increment)
plot_band(ax, t_post, m[bi:], s[bi:], lo[bi:], hi[bi:], C_RND)
style_ax(ax, "Productivity Increment (z) per Firm", ylabel="Increment")

# (1,1) Percentage of firms doing R&D
ax = axes[1, 1]
ax.plot(t_post, pct_rnd_active[bi:], color=C_RND, linewidth=1)
ax.set_ylim(0, 100)
style_ax(ax, "Firms Doing R&D (sigma > 0)", ylabel="%")

# (2,0) R&D fragility per firm (wage_bill / net_worth)
ax = axes[2, 0]
m, s, lo, hi = weighted_stats(rnd_fragility)
plot_band(ax, t_post, m[bi:], s[bi:], lo[bi:], hi[bi:], C_RND)
style_ax(ax, "R&D Fragility (W/A) per Firm", ylabel="Ratio")

# (2,1) Total R&D expenditure
ax = axes[2, 1]
ax.plot(t_post, total_rnd_expenditure[bi:], color=C_RND, linewidth=1)
style_ax(ax, "Total R&D Expenditure (sigma * net_profit)", ylabel="Amount")

# (3,0) Labor productivity distribution (KDE snapshots)
plot_kde_snapshots(
    axes[3, 0], labor_productivity, SNAPSHOT_TIMES, "Labor Productivity (KDE)"
)

# (3,1) R&D sigma distribution (KDE snapshots)
plot_kde_snapshots(axes[3, 1], rnd_sigma, SNAPSHOT_TIMES, "R&D Share (KDE)")

plt.tight_layout()
save_figure(fig, 14, "rnd_dynamics")

# %%
# Figure 15: Productivity & Wage Co-movement (Growth+ Specific)
# ---------------------------------------------------------------

fig, axes = plt.subplots(4, 2, figsize=(14, 20))
fig.suptitle(
    "Figure 15: Productivity & Wage Co-movement (Growth+)", fontsize=14, y=0.995
)

# (0,0) Productivity and real wage (dual axis)
ax = axes[0, 0]
ax.plot(t_post, avg_productivity[bi:], color=C_RND, linewidth=1.2, label="Productivity")
ax.plot(t_post, real_wage[bi:], color=C_LABOR, linewidth=1.2, label="Real Wage")
ax.legend(fontsize=8)
style_ax(ax, "Productivity vs Real Wage", ylabel="Level")

# (0,1) Wage-to-productivity ratio
ax = axes[0, 1]
ax.plot(t_post, wage_prod_ratio[bi:], color=C_PRICE, linewidth=1)
ax.axhline(1.0, color="gray", linewidth=0.5, linestyle=":")
style_ax(ax, "Wage / Productivity Ratio", ylabel="Ratio")

# (1,0) Productivity growth rate
ax = axes[1, 0]
ax.plot(t_post[1:], productivity_growth[bi:] * 100, color=C_RND, linewidth=0.8)
ax.axhline(0, color="gray", linewidth=0.5, linestyle=":")
style_ax(ax, "Productivity Growth Rate", ylabel="%")

# (1,1) Log GDP with trend
ax = axes[1, 1]
ax.plot(t_post, log_gdp[bi:], color=C_PROD, linewidth=1)
# Add linear trend
t_axis = np.arange(len(log_gdp[bi:]))
try:
    slope, intercept = np.polyfit(t_axis, log_gdp[bi:], 1)
    ax.plot(
        t_post,
        slope * t_axis + intercept,
        "r--",
        linewidth=1,
        alpha=0.7,
        label=f"Trend: {slope:.5f}/period",
    )
    ax.legend(fontsize=8)
except np.linalg.LinAlgError:
    pass
style_ax(ax, "Log GDP with Trend", ylabel="Log output")

# (2,0) Productivity vs GDP growth scatter
scatter_regression(
    axes[2, 0],
    productivity_growth[bi:],
    gdp_growth[bi:],
    np.arange(bi + 1, N_PERIODS),
    "Productivity Growth vs GDP Growth",
    "Productivity Growth",
    "GDP Growth",
)

# (2,1) Sigma vs fragility scatter (final period)
ax = axes[2, 1]
valid = rnd_fragility[-1] > 0
ax.scatter(
    rnd_fragility[-1][valid],
    rnd_sigma[-1][valid],
    s=15,
    alpha=0.6,
    color=C_RND,
...
style_ax(
    ax,
    "R&D Share vs Fragility (final period)",
    xlabel="Fragility (W/A)",
    ylabel="Sigma",
)

# (3,0) Cumulative productivity growth
cumulative_prod_growth = (avg_productivity[bi:] - avg_productivity[bi]) / max(
    avg_productivity[bi], 1e-10
)
ax = axes[3, 0]
ax.plot(t_post, cumulative_prod_growth * 100, color=C_RND, linewidth=1.2)
style_ax(ax, "Cumulative Productivity Growth", ylabel="%")

# (3,1) Real wage growth vs productivity growth
cumulative_wage_growth = (real_wage[bi:] - real_wage[bi]) / max(real_wage[bi], 1e-10)
ax = axes[3, 1]
ax.plot(
    t_post,
    cumulative_prod_growth * 100,
    color=C_RND,
    linewidth=1.2,
    label="Productivity",
)
ax.plot(
    t_post,
    cumulative_wage_growth * 100,
    color=C_LABOR,
    linewidth=1.2,
    label="Real Wage",
)
ax.legend(fontsize=8)
style_ax(ax, "Cumulative Growth: Productivity vs Real Wage", ylabel="%")

plt.tight_layout()
save_figure(fig, 15, "productivity_wage")

# %%
# Summary
# -------

print(f"\nDiagnostics complete! {15} figures saved to {OUTPUT_DIR}/")
...
print(
    f"  Avg productivity growth: {(avg_productivity[-1] - avg_productivity[bi]) / max(avg_productivity[bi], 1e-10) * 100:.1f}%"
)
print(f"  Avg R&D intensity: {np.mean(avg_rnd_intensity[bi:]):.6f}")
