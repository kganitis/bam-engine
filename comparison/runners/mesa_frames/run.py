"""mesa-frames runner for the comparison harness.

This is the mesa-frames subprocess runner: reads a :class:`RunRequest` JSON
file (path in ``sys.argv[1]``), runs the mesa-frames BAM model, and prints a
...
def main(request_path: str) -> None:
    """Run the mesa-frames BAM model for one request and print a RunResult JSON to stdout."""
    with open(request_path) as fh:
        req = RunRequest.from_json(fh.read())

    framework_version = getattr(mf, "__version__", "unknown")
