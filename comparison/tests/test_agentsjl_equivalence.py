"""Committed equivalence-gate acceptance test for the Agents.jl port.

Runs the full gate phase (20 seeds × 1000 periods) for both ``bamengine`` and
``agentsjl``, then asserts that the Agents.jl port passes all six behavioral
equivalence metrics.

Marked ``@pytest.mark.slow`` -- not included in the quick regression suite.
Skipped automatically if the ``julia`` executable is not on PATH.

Expect this to be SLOW: 20 Agents.jl simulations of 1000 periods each, plus 20
bamengine reference runs.  Each Julia subprocess JIT-compiles the model on
startup, so the per-job startup cost is non-trivial.
"""
...
# Per-job wall-clock budget: 1000-period Agents.jl runs at 100 firms include a
# one-off JIT-compilation cost per process; 600 s gives a generous margin.
_BUDGET_S = 600.0
_MAX_WORKERS = 8
_BURN_IN = 500
...
@pytest.mark.skipif(
    ...
    reason="julia executable not on PATH",
)
def test_agentsjl_passes_equivalence_gate():
    """agentsjl must pass all six behavioral equivalence metrics at the full gate size.
    ...
    assert by_fw["agentsjl"], (
        f"No successful agentsjl runs -- cannot evaluate gate.{run_failure_summary}"
    )

    gate = gate_mod.evaluate_gate(by_fw)
    aj_result = gate["frameworks"]["agentsjl"]
    ...
    failing_metrics = [
        ...
        f"  (bamengine_mean={info['bamengine_mean']:.4f}  agentsjl_mean={info['mean']:.4f})"
        for m, info in aj_result["metrics"].items()
    ...
    assert aj_result["passed"], (
        f"agentsjl did NOT pass the equivalence gate.{failure_detail}{run_failure_summary}"
    )
