"""Committed equivalence-gate acceptance test for the Mesa port.

Runs the full gate phase (20 seeds × 1000 periods) for both ``bamengine`` and
``mesa``, then asserts that Mesa passes all six behavioral equivalence metrics.

Marked ``@pytest.mark.slow`` -- not included in the quick regression suite.
Expect a few minutes to complete (20 Mesa simulations of 1000 periods each run
...
# Per-job wall-clock budget: 1000-period Mesa runs at 100 firms take roughly
# 60-90 s on a typical laptop; 600 s gives a generous safety margin.
_BUDGET_S = 600.0
_MAX_WORKERS = 8
_BURN_IN = 500
...
@pytest.mark.slow
def test_mesa_passes_equivalence_gate():
    """Mesa must pass all six behavioral equivalence metrics at the full gate size.
    ...
    assert by_fw["mesa"], (
        f"No successful mesa runs -- cannot evaluate gate.{run_failure_summary}"
    )

    gate = gate_mod.evaluate_gate(by_fw)
    mesa_result = gate["frameworks"]["mesa"]
    ...
    failing_metrics = [
        ...
        f"  (bamengine_mean={info['bamengine_mean']:.4f}  mesa_mean={info['mean']:.4f})"
        for m, info in mesa_result["metrics"].items()
    ...
    assert mesa_result["passed"], (
        f"Mesa did NOT pass the equivalence gate.{failure_detail}{run_failure_summary}"
    )
