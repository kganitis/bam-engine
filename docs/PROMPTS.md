It's time to run our first simulation to cross check the results with the paper.
To do so, we will need to generate some plots.
We will run the baseline scenario of the model and present into plots:
1. the (log) **real** GDP: horizontal axis: t (periods); vertical axis: Log REAL GDP
2. the rate of unemployment: horizontal axis: t (periods); vertical axis: unemployment rate
3. the annual inflation rate: horizontal axis: t (periods); vertical axis: annual inflation rate
4. the ratio of labor productivity to the real wage: horizontal axis: t (periods); vertical axis: ratio of labor productivity to the real wage
The simulation will be run for the default parameters, and:
500 consumers (households), 100 firms, 10 banks and 1000 periods.
In order to get rid of the transients, we will consider only the last 500 simulated periods
I guess we need to somehow capture the needed values from each period to present them to plots.
Your output should be a new file that will use the scheduler.
Also take care to include logging for debugging reasons, if the plots fail to show the expected output.
This is the `scheduler.py` file. Tell me if you need another file for reference, if you aren't sure how something is implemented or calculated.
``` 
