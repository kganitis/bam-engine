...
class TestComplexConditionalValidation:
    """Validate complex conditional logic using ops."""

    def test_multi_branch_logic_ops_matches_numpy(self):
        """
        Validate complex multi-branch conditional logic.
        ...
        avg_price = 100.0
        current_prod = rng.uniform(50, 150, 100)

        # NumPy implementation: complex production decision
        # Case 1: High inventory (>30) → cut production 20%
        # Case 2: No inventory + high price → increase 15%
        # Case 3: Low inventory (1-10) + low price → cut 10%
        # Default: keep same

        cond1_np = inventory > 30
        cond2_np = np.logical_and(inventory == 0, price >= avg_price)
        cond3_np = np.logical_and(
            np.logical_and(inventory > 0, inventory <= 10), price < avg_price
        )

        choice1_np = current_prod * 0.8
        choice2_np = current_prod * 1.15
        choice3_np = current_prod * 0.9

        result_np = np.select(
            [cond1_np, cond2_np, cond3_np],
            [choice1_np, choice2_np, choice3_np],
            default=current_prod,
        )

        # ops implementation
        cond1_ops = ops.greater(inventory, 30)
        cond2_ops = ops.logical_and(
            ops.equal(inventory, 0), ops.greater_equal(price, avg_price)
        )
        cond3_ops = ops.logical_and(
            ops.logical_and(ops.greater(inventory, 0), ops.less_equal(inventory, 10)),
            ops.less(price, avg_price),
        )

        choice1_ops = ops.multiply(current_prod, 0.8)
        choice2_ops = ops.multiply(current_prod, 1.15)
        choice3_ops = ops.multiply(current_prod, 0.9)

        result_ops = ops.select(
            [cond1_ops, cond2_ops, cond3_ops],
            [choice1_ops, choice2_ops, choice3_ops],
            default=current_prod,
        )
        ...
        np.testing.assert_array_almost_equal(
            ...
            result_ops,
            decimal=10,
            err_msg="ops-based multi-branch logic doesn't match NumPy implementation",
        )
