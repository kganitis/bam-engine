...
def compute_growth_plus_metrics(
    ...
    real_interest_rate_bounds: tuple[float, float] = (-0.02, 0.12),
) -> GrowthPlusMetrics:
    ...
    return GrowthPlusMetrics(
        ...
        real_interest_rate_pct_in_bounds=float(
            np.mean(
                (real_ir_ss >= real_interest_rate_bounds[0])
                & (real_ir_ss <= real_interest_rate_bounds[1])
            )
        ),
...
METRIC_SPECS = [
    ...
    MetricSpec(
        ...
        weight=2.0,  # Structural validity
        group=MetricGroup.FINANCIAL,
        threshold=1.0,
...
def _compute_metrics_wrapper(
    ...
    rir_bounds = metrics["real_interest_rate_pct_in_bounds"]
    ...
    return compute_growth_plus_metrics(
        ...
        real_interest_rate_bounds=(rir_bounds["bounds_min"], rir_bounds["bounds_max"]),
    )
