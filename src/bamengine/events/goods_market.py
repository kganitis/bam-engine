...
class ConsumersCalcPropensity:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        log = self.get_logger()
        con = sim.con
        avg_sav = float(con.savings.mean())
...
class ConsumersDecideFirmsToVisit:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if avail.size == 0:
            log.info(
                "  No firms have inventory available. All shopping queues cleared."
            )
        ...
        for h in range(con.income_to_spend.size):
            ...
            if loyal:
                ...
                if log.isEnabledFor(logging.DEEP_DEBUG):
                    log.deep(
                        f"    Consumer {h}: Applied loyalty to firm {prev} (slot 0)"
                    )
            ...
            if loyal and filled > 1 and row[0] != prev:  # pragma: no cover
                log.warning(
                    f"    Consumer {h}: Loyalty violation detected, correcting..."
                )
        ...
        # Summary statistics
        avg_selections = (
            total_selections_made / consumers_processed
            if consumers_processed > 0
            else 0.0
        )
        loyalty_rate = (
