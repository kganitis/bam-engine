...
class BanksDecideCreditSupply:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        log = self.get_logger()  # type: ignore[attr-defined]
        lend = sim.lend
        v = sim.ec.v
...
class FirmsPrepareLoanApplications:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        sample = np.array(
            [
                rng.choice(lenders, size=H_eff, replace=False)
                for _ in range(borrowers.size)
            ]
        )
        if log.isEnabledFor(logging.DEBUG):
            log.debug(
...
class FirmsSendOneLoanApp:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        log.info(
            f"  Processing {borrowers_applying.size}"
            f" borrowers with pending applications"
            f" (Stride={stride})."
        )
        ...
        for i in borrowers_applying:
            ...
            if head >= (i + 1) * stride:
                ...
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        f"    Borrower {i} has exhausted all {stride} slots. "
                        f"Setting head to -1."
                    )
...
class BanksProvideLoans:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        def _clean_queue(slice_: np.ndarray, bank_idx_for_log: int) -> np.ndarray:
            """
            Return a queue of *unique* borrower ids that still demand credit
        ...
        for k in bank_ids:
            if log.isEnabledFor(logging.DEBUG):
                log.debug(
                    f"  Processing bank {k} (credit supply: {lend.credit_supply[k]})"
                )
            ...
            queue = _clean_queue(raw_queue, bank_idx_for_log=k)
            ...
            if cut.any():
                ...
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        f"    Bank {k} credit supply exceeded"
                        f" at position {first_exceed}. "
            ...
            if not mask.any():  # pragma: no cover
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        f"    Bank {k}: No loans granted this round. Flushing queue."
                    )
                lend.recv_loan_apps_head[k] = -1
                lend.recv_loan_apps[k, :n_recv] = -1
                continue
...
class FirmsFireWorkers:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        wrk = sim.wrk
        rng = sim.rng
        # Default to "expensive", check config if available
        method = getattr(sim.config, "firing_method", "expensive")
        ...
        for i in firing_ids:
            ...
            emp.wage_bill[i] = (
                wrk.wage[remaining_workforce].sum()
                if remaining_workforce.size > 0
                else 0.0
            )
