...
# TODO Move to a common place for all modules to use
#  Same for all event modules in this package.
_EPS = 1.0e-9
...
class FirmsCalcBreakevenPrice:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if log.isEnabledFor(logging.DEBUG):
            valid = breakeven[np.isfinite(breakeven)]
            log.debug(
                f"  Raw breakeven prices (before cap): "
                f"min={valid.min() if valid.size > 0 else 'N/A':.2f}, "
                f"max={valid.max() if valid.size > 0 else 'N/A':.2f}, "
                f"avg={valid.mean() if valid.size > 0 else 'N/A':.2f}"
            )

        # Cap breakeven
        if cap_factor and cap_factor > 1:
            # Cannot be more than current price x cap_factor.
            # This prevents extreme jumps.
...
class FirmsAdjustPrice:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if log.isEnabledFor(logging.DEBUG):
            if n_up > 0:
                log.debug(
                    f"    Firms increasing price: {np.where(mask_up)[0].tolist()}"
                )
            if n_dn > 0:
                log.debug(
                    f"    Firms decreasing price: {np.where(mask_dn)[0].tolist()}"
                )
