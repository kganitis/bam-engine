...
class CalcAnnualInflationRate:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        p_now = hist[-1]
        p_prev = hist[-5]  # Price from 4 periods ago

        if p_prev <= 0:
            log.warning(
                "  Cannot calculate inflation,"
                " previous price level was zero or negative."
                " Setting to 0.0."
            )
            inflation = 0.0
        else:
        ...
        log.info(
            f"  Annual inflation calculated for period t={hist.size - 1}:"
            f" {inflation:+.3%}"
        )
...
class AdjustMinimumWage:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if ec.avg_mkt_price_history.size <= m:
            log.debug(
                f"  Skipping: not enough history"
                f" ({ec.avg_mkt_price_history.size} <= {m})."
            )
            return
...
class WorkersSendOneRound:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        for j in unemp_ids_applying:
            ...
            if log.isEnabledFor(logging.DEBUG):
                log.debug(
                    f"    Worker {j} applying to firm {firm_id} (app #{col + 1})."
                )
...
class FirmsHireWorkers:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        # Helper function for labor consistency checks
        def _check_labor_consistency(tag: str, i: int) -> bool:
            """Verify that recorded current_labor matches worker table."""
            true_headcount = np.count_nonzero((wrk.employed == 1) & (wrk.employer == i))
            recorded = int(emp.current_labor[i])
            if true_headcount != recorded:
                log.error(
                    f"[{tag}] LABOR MISMATCH for firm {i}: "
                    f"recorded={recorded}, true={true_headcount}"
                )
                return False
            if log.isEnabledFor(logging.DEBUG):
                log.debug(
                    f"[{tag}] Firm {i} labor check OK: "
                    f"recorded={recorded}, true={true_headcount}"
                )
            return True

        # Helper function to clean application queue
        def _clean_queue(slice_: np.ndarray, firm_idx_for_log: int) -> np.ndarray:
            """
            Remove duplicates and already-employed workers from raw application queue.
            Returns clean array of valid worker indices.
            """
            unique_apps = np.unique(slice_)
            if unique_apps.size < slice_.size:
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        f"    Firm {firm_idx_for_log}: removed "
                        f"{slice_.size - unique_apps.size} duplicate application(s)"
                    )

            unemployed_filter = wrk.employed[unique_apps] == 0
            num_already_employed = np.sum(~unemployed_filter)
            if num_already_employed > 0:
                if log.isEnabledFor(logging.DEBUG):
                    log.debug(
                        f"    Firm {firm_idx_for_log}: removed "
                        f"{num_already_employed} already-employed applicant(s)"
                    )

            clean = unique_apps[unemployed_filter]
            return clean

        # Helper function to safely count employed workers per firm
        def _safe_bincount_employed(n_firms: int) -> np.ndarray:
            """
            Return array of true labor counts per firm based on worker table.
            Handles edge cases (all unemployed, no workers, etc.).
            """
            employed_mask = wrk.employed == 1
            if not np.any(employed_mask):
                if log.isEnabledFor(logging.DEBUG):
                    log.debug("  No employed workers in entire economy.")
                return np.zeros(n_firms, dtype=np.int64)

            employer_vals = wrk.employer[employed_mask]
            if employer_vals.size == 0:
                return np.zeros(n_firms, dtype=np.int64)

            counts = np.bincount(employer_vals, minlength=n_firms)
        ...
        for i in hiring_ids:
            ...
            _check_labor_consistency("PRE-hire", i)
            ...
            queue = _clean_queue(raw_queue, firm_idx_for_log=i)
            ...
            _check_labor_consistency("POST-hire", i)

        log.info(
            f"  Total hires made this step across all firms: {total_hires_this_round}"
        )
        if log.isEnabledFor(logging.DEBUG):
            true_labor_counts = _safe_bincount_employed(emp.current_labor.size)
            ...
            if mismatched_firms.size:
                ...
                for i_mismatch in mismatched_firms:
                    log.error(
                        f"  Firm {i_mismatch}:"
                        f" recorded={emp.current_labor[i_mismatch]},"
...
class FirmsCalcWageBill:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if log.isEnabledFor(logging.DEBUG):
            log.debug(f"  Wage bill per firm (first 10): {emp.wage_bill[:10]}")

        log.info("--- Firms Calculating Wage Bill complete ---")
