...
class FirmsCollectRevenue:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if log.isEnabledFor(logging.DEBUG):
            log.debug(
                f"  Total funds (cash) before revenue collection: {bor.total_funds}"
            )
        ...
        if log.isEnabledFor(logging.DEBUG):
            log.debug(
                f"  Total funds (cash) after revenue collection: {bor.total_funds}"
            )
...
class FirmsValidateDebtCommitments:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if log.isEnabledFor(logging.DEBUG):
            ...
            log.debug(
                f"  Borrower total funds before debt validation: {bor.total_funds}"
            )
        ...
        if repay_firms.size > 0:
            ...
            log.debug(
                f"  Aggregating {num_loans_repaid} loan repayments to lender equity."
            )
        ...
        if bad_firms.size > 0:
            log.info(
                ...
                f"{bad_firms.size} defaulting firms."
            )

            # zero out cash for defaulting firms
            log.info(
                f"  Zeroing out total_funds for {bad_firms.size} defaulting firms."
            )
            ...
            if log.isEnabledFor(logging.DEBUG):
                ...
                for firm_idx in sample_default_firms:
                    log.debug(
                        f"    Firm {firm_idx}: "
                        f"total_funds changing from "
                        f"{bor.total_funds[firm_idx]:.1f} to 0.0"
                    )
            ...
            if np.any(bad_rows_in_lb_mask):
                ...
                log.debug(
                    f"  {num_bad_loans} loans in loanbook "
                    f"belong to these defaulting firms."
                )

                # calculate proportional write-offs
                d_tot_map = total_debt[borrowers_from_lb[bad_rows_in_lb_mask]]
                frac = lb.debt[: lb.size][bad_rows_in_lb_mask] / np.maximum(
                    d_tot_map, _EPS
                )
...
class FirmsPayDividends:
    ...
    def execute(self, sim: Simulation) -> None:
        ...
        if log.isEnabledFor(logging.DEBUG):
            ...
            log.debug(
                f"  Total funds (cash) before paying dividends: {bor.total_funds}"
            )
